...
import { collection, query, where, getDocs, getDoc, limit, orderBy, addDoc, doc, updateDoc } from "firebase/firestore";
...
export default function FindTeacherForm({ onViewChange }: { onViewChange?: (view: string) => void }) {
  ...
  useEffect(() => {
    const fetchTeachers = async () => {
      ...
      try {
        const teachersRef = collection(db, "teacherProfiles");
        const q = query(teachersRef, limit(50));
        const querySnapshot = await getDocs(q);

        console.log(`📊 Found ${querySnapshot.docs.length} teacher profiles`);
        const teachersData: Teacher[] = [];

        // Fetch all teacher profiles with their user data
        for (const docSnap of querySnapshot.docs) {
          const data = docSnap.data();
          console.log(`Processing teacher ${docSnap.id}:`, data);
          ...
          try {
            ...
            if (userDoc.exists()) {
              teacherName = userDoc.data().name || teacherName;
              teacherEmail = userDoc.data().email || "";
              console.log(`✅ Found user data for ${docSnap.id}:`, teacherName);
            } else {
              console.warn(`⚠️ No user document found for teacher ${docSnap.id}`);
            }
          } catch (e) {
            console.error("Error fetching user data for teacher:", docSnap.id, e);
          }

          // Handle empty strings by treating them as falsy
          const city = data.city?.trim() || "";
          const district = data.district?.trim() || "";
          const location = city || district || "Nepal";

          teachersData.push({
            id: docSnap.id as any,
            name: teacherName,
            subjects: data.subjects ? data.subjects.split(", ") : [],
            rate: parseInt(data.rateAmount) || 0,
            rateType: data.rateType === "monthly" ? "monthly" : "hourly",
            rating: data.rating || 0,
            reviews: data.reviews || 0,
            location: location,
            teachingMode: data.teachingMode || "Online",
            experience: data.experience || "N/A",
            qualifications: data.highestQualification || "N/A",
            image: `https://ui-avatars.com/api/?name=${encodeURIComponent(teacherName)}&background=random`,
            whatsappEnabled: data.whatsappConsent || false,
            email: teacherEmail,
        ...
        console.log(`✅ Successfully processed ${teachersData.length} teachers`);
        console.log("Teachers data:", teachersData);

        if (teachersData.length === 0) {
          console.warn("⚠️ No teachers found in database, showing mock data");
          toast.info("No teachers registered yet. Showing sample data.");
          setFilteredTeachers(mockTeachers);
        } else {
          setFilteredTeachers(teachersData);
          toast.success(`Found ${teachersData.length} teacher${teachersData.length > 1 ? 's' : ''}`);
        }
      } catch (error: any) {
        console.error("❌ Error fetching teachers:", error);
        console.error("Error stack:", error.stack);
        toast.error("Failed to load teachers: " + error.message);
        setFilteredTeachers(mockTeachers); // Fallback
      } finally {
        setLoading(false);
      }
    };
...
const checkAuth = () => {
  ...
  if (!user) {
    toast.error("Sign in required to view details or contact teachers");
    if (onViewChange) {
...
const trackInteraction = async (teacherId: string | number) => {
  ...
  // Firebase persistence if logged in
  const user = auth.currentUser;
  if (user) {
    try {
...
const hasInteracted = (teacherId: string | number) => {
  const interactions = JSON.parse(localStorage.getItem("hamroguru_interactions") || "{}");
...
const TeacherDetailModal = ({ teacher, onClose }: { teacher: Teacher; onClose: () => void }) => {
  ...
  const handleRateSubmit = async (r: number) => {
    const user = auth.currentUser;
    if (!user) {
    ...
    try {
      ...
      // 2. Optimistically update teacher profile in Firestore
      const teacherRef = doc(db, "teacherProfiles", String(teacher.id));
      const newReviews = (teacher.reviews || 0) + 1;
      const newRating = Number((((teacher.rating || 0) * (teacher.reviews || 0) + r) / newReviews).toFixed(1));
